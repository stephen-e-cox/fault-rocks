...
#%% Properties to define before running the model
...
#%% Sort out measurement data and start thermal model calculations
...
''' Calculate background maturity from bg measurements OR define a background 
separately'''

''' Calculating background maturity'''
''' Define a distance away from slip layer to distinguish background samples'''
bg_thresh = 0.1

''' Extract background maturities and calculate an average background'''
bg_vect = []
...
''' Define whether we use the average bg calculated above or define it here '''
MPI4_bg = 0.5056


''' Set up distance x time grid '''
Ng = 50
...
''' define the second intergral if hte inverse error function '''
def i2erfc(z):
...
for fric in mu:
    for a in halfwidth:
        for disp in slip:
            ...
            ''' temperature required for reaction if relevant '''
            '''Trxnlim1 = 180'''
...
''' Use the analytical uncertainty (1-sigma) in MPI-4 to set up reaction bounds
    (this needs to be requantified) '''
sigma = 0.038/2

''' Reacted thermal maturities to target '''
...
for targ in MPI4_target:
    ...
    ''' Filter out relevant maturities and other parameters, other conditions 
    can be added here '''
    hit_idx = np.where((MPI4max_rs[:,0] >= MPI4_target[targ_idx] -2*sigma) & (MPI4max_rs[:,0] <= MPI4_target[targ_idx] + 2*sigma))[0]
...
''' Loop through the target maturities and create temperature and thermal maturity
    profiles for each '''
...
for targ in MPI4_target:
    ...
    for step in rs_loop:
        ...
        if Tprofile_hits[0, step, targ_idx] != 0:
            ...
            plt.plot(x, Tprofile_hits[:, step, targ_idx], color = 'lightsteelblue')
            
    
    plt.plot(x, Tprofile_ave, color = 'red')
            
    plt.xlim([min(x), max(width) + 0.01])
    ...
    plt.figure()
    plt.title('Thermal maturity profiles matching MPI4 = ' + str(targ))
    plt.xscale('log')
    ...
    for step in rs_loop:
        ...
        if MPI4profile_hits[0, step, targ_idx] != 0:
            ...
            plt.plot(x, MPI4profile_hits[:, step, targ_idx], color = 'lightsteelblue')
